...
# 主方法数体
def main():
    parser = optparse.OptionParser('Usage%prog -i <single ip address> -I <multi ip address> \n  \n{ Egg: python getIpLocation.py -i(I) 100.8.11.58(100.8.11.58-100) }')
    parser.add_option('-i', dest='SIp',type='string', help='specify pcap filename')
    parser.add_option('-I', dest='MIp',type='string', help='specify pcap filename')
    (options, args) = parser.parse_args()
    if options.SIp == None and options.MIp == None:
        print (parser.usage)
        exit(0)
    ip_address  = options.SIp
    ip_address_multi  = options.MIp
    if ip_address==None:
        seperate_ip(ip_address_multi)
    else:
        seperate_ip(ip_address)
